...
Embedded modern Web UI & Admin Control Center driving adapter. Stdlib `net/http` only — no chi/gin/echo — to match the project's minimal-deps posture. Bound to a loopback address by default; reverse-proxy TLS termination is the documented path for remote access. There is no built-in TLS.

The UI is a dual-mode application (💬 **Web Chat** and ⚡ **Admin Control Center**):
- **Web Chat (`/chat`)**: Full-featured interactive chat interface for collaborating with OpenHarness directly in the browser. Features auto-scrolling conversation feed, real-time message stream polling (`/admin/fragments/chat_messages`), collapsible tool execution accordions with argument and output inspectors, quick suggestion chips, live thinking/executing loop status pills, and keyboard shortcut composers (Enter to send, Shift+Enter for new line).
- **Admin Control Center (`/admin`)**: Sleek dark dashboard with tabs for System Overview (live phase, token compaction gauge, chat latency), Memory Explorer (`/admin/memory`, interactive file tree and rendered markdown previews), Skills Catalog (`/admin/skills`), Subagents Hub (`/admin/subagents`), Tool Execution Trace (`/admin/tools`), Configuration Editor (`/admin/configuration`), Version & Self-Update (`/admin/version`), and Live Logs Tailing (`/admin/logs`).

- **`server.go`**: `Server`, `New`, `Run`, `Shutdown`, route registration, `SetInspectors`, `SetSkillsAdmin`, `SetMemoryAdmin`. Each (layout, content) pair is parsed once at startup into its own `*template.Template`. `cacheImmutable` middleware tags `/admin/static/` responses with `Cache-Control: public, max-age=31536000, immutable`.
- **`middleware.go`**: `requireAuth` (session lookup + CSRF check on non-safe methods), `requestLogger` (per-request structured log line). Session cookie uses `Path: /` so authenticated sessions cover `/chat`, `/`, and `/admin/*`.
- **`handlers_chat.go`**: `GET /chat`, `POST /chat/send`, `GET /admin/fragments/chat_messages`, `GET /admin/fragments/chat_status`. Pairs LLM tool-call invocations with their responses into clean expandable UI accordions and formats Markdown prose.
- **`handlers_memory.go`**: `GET /admin/memory`, `GET /admin/fragments/memory_file`. Lists on-disk memory files and renders live formatted previews.
- **`handlers_tools.go`**: `GET /admin/tools`. Dedicated full-page view for the in-memory ring buffer of tool calls.
- **`handlers_auth.go`**: `GET/POST /admin/login`, `POST /admin/logout`. Modern glassmorphism dark card with transient pre-session CSRF token validation.
- **`page.go`**: `pageData` is the common base struct every authenticated page embeds, tracking `TopTab` ("chat" vs "admin"), navigation items, and live loop status telemetry.
- **`assets/`**: vendored `htmx.min.js`, `tailwind.js`, and `modern.css` (custom design system inspired by Linear/Vercel with glowing status dots, chat bubbles, collapsible cards, and responsive grids). Embedded via `go:embed`.
- **`templates/`**: `layout.html`, `login.html`, `chat.html`, `memory.html`, `tools.html`, `dashboard.html`, `configuration.html`, `subagents.html`, `skills.html`, `version.html`, `logs.html`, plus fragment templates. Embedded via `go:embed`.
